...
read.gff <- function (x,name="score") {
  fn.ext <- file_ext(x)                                                                                                 ##identifies the extension of the file as alphanumeric string
  
  if (grepl("gff",ignore.case=T,fn.ext)) {                                                                              ##'ignore.case=T' indicates to treat small & capital letters similarly
	temp.data <- read.table(x,row.names=NULL)
	if (ncol(temp.data) > 5) {                                                                                            ##checking only for 'ncol()>5' implies that only 'gff'- or 'bedgraph'-files would be parsed 
	  # GFF
	...
	temp.data <- read.table(x,row.names=NULL,skip=1)                                                                     ##skip indicates to ignore the header of the bedgraph-file
	if (ncol(temp.data) == 4) {
		# bedgraph
		trim.data <- temp.data
	} else {
...
gene.exp <- function (input.df, buffer=0, iter=50000, debug=F) {
  ...
  avg.exp <- avg.exp[0,]                                                                                              ##reset the entire dataframe
  ...
  for (thres in thres.samp) {
    ...
    # init vars
    for (f in frag.samp) {                                                                                           ##create list of different threshold-set-ups
      # List names are, e.g. frag 1, thres 0.2: rand[[thres.1.0.2]]
      rand[[paste("thres.",f,".",thres,sep="")]] <- 0;
    }
    
    for (i in 1:iter) {
      if (i %% 200 == 0) {cat(paste("  iter",i,"\r"))}                                                               ##TRUE, whenever i is multiple of 200
      ...
      for (f in frag.samp) {
        ...
        rand.samp[[paste("rand.",f,sep="")]] <- mean(input.df[runif(f,1,input.len),4])                               ##pick f-number of random values from the entire column 4 (i.e., "val") in the *.bedgraph-file, calculate the mean &
      }                                                                                                              ##store in the 'rand.sample'-list --> for every iteration, a new set of 9 values (=frag.sample) generated
      
      # count number of times exp > thres
      for (f in frag.samp) {                                                                                         ##register the number of times the mean of the randomized sampling exceeds the specified threshold
  ...
  for (thres in thres.samp) {
    for (f in frag.samp) {                                                                                          ##calculate the normalized counts by dividing them with the amount of iterations used
  ...
  for (thres in thres.samp) {
    for (f in frag.samp) {                                                                                         ##enhances the newly generated var.thres by the 'f'-related corresponding entries for rand.fdr, i.e., concatenation
  ...
  var.lm <- list()
  for (f in frag.samp) {                                                                                          ##parses all values from rand.fdr/var.thres to the inf.log.lm-function, where these values are correlated with the thres.sample-values
  ...
  for (chromo in unique(genes$chr)) {                                                                             ##unfolds only the chromosomes that encode for the genes of interest, loops through the individual chromosomes
    input.chr <- subset(input.df, chr==chromo)                                                                    ##subset the current bedgraph-file to the current chromosome
    genes.chr <- subset(genes, chr==chromo)                                                                       ##subset the gene-list to the current chromosome
    for (i in 1:length(genes.chr$name)) {                                                                         ##loop through the genes encoded on the current chromosome
      ...
      gene.start <- genes.chr[i,"start"] - buffer                                                                 ##extend the start & end position of the current gene by a predefined margin
      gene.end <- genes.chr[i,"end"] + buffer
      
      gene.start <- ifelse(gene.start < 1, 1, gene.start)                                                         ##if the gene-start-side is less than 1 due to the extension by the predefined margin, set the start-position back to '1'
      
      # Create data frames for all gatc fragments covering current gene
      exp <- data.frame(input.chr[(input.chr$start <= gene.end) & (input.chr$end >= gene.start),] )               ##subsets only the GATC-fragments falling fully into the gene body (inbetween the start- & end-coordinates)
      ...
      exp$start[1] <- gene.start                                                                                  ##extend the first and last GATC-fragment within the gene body to the start and end positions of the actual gene
      exp$end[length(exp[,1])] <- gene.end
      
      # gene length covered by gatc fragments
      len <- sum(exp$end-exp$start)                                                                               ##similar to 'exp$end[nrow(exp)]-exp$start[1]'
      
      # calculate weighted score for each column (representing different proteins)
      for (j in 4:length(names(input.chr))) {
        avg[j] <- (sum((exp$end-exp$start)*exp[j]))/len                                                           ##weigh the intensity associated to the individual GATC-fragments according to their length, sum these weighted fragments & divide by the entire gene length
      }
     
      # make data.frame of averages (to be appended to avg.exp)                                                   ##concatenate the weighted averages for the gene (gene_isoforms) in a dataframe by cbind
      df <- cbind(avg[1])
      for (k in 2:length(avg)) {
        df <- cbind(df,avg[k])
      }
      df <- cbind(df,gatc.num)                                                                                    ##add the maximum number of GATC-fragments associated with this particular gene (and its isoforms)
      
      # only fdr for first column for now ...
      gene.fdr <- fdr(gatc.num,avg[4])                                                                            ##invoke the fdr-function (see above) for this particular gene & the amount of GATC-fragments associated with it
      df <- cbind(df, gene.fdr)                                                                                   ##include it into the dataframe via cbind
      
      # append current gene to list
      avg.exp <- rbind(avg.exp,data.frame(name=as.character(genes.chr[i,"name"]), df))                            ##concatenate the information for the current gene to the avg.exp-dataframe
      count <- count+1
